...
function loadData(){global $dataFile;if(file_exists($dataFile)){$d=json_decode(file_get_contents($dataFile),true);if($d&&is_array($d['groups']??null))return $d;}return['adminHash'=>'44f61792d66021c0030fa37dca5162871345c525f61984b88fa1af16d8117672','siteName'=>'StarNav','siteDesc'=>'StarNav - 最实用的经验，分享最需要的你','groups'=>[['id'=>'default','name'=>'默认','icon'=>'home','bookmarks'=>[]]]];}
...
'umbrella2'=>['path d="M23 12a11.05 11.05 0 00-22 0','path d="M12 12v10','path d="M5 12.18a5.79 5.79 0 015-3.43 5.79 5.79 0 015 3.43'],
...
'hash2'=>['line x1="4" y1="9" x2="20" y2="9','line x1="4" y1="15" x2="20" y2="15','line x1="10" y1="3" x2="8" y2="21','line x1="16" y1="3" x2="14" y2="21'],
];
?>
<!DOCTYPE html><html lang="zh-CN" data-theme="dark"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?=esc($data['siteName'])?></title><meta name="application-name" content="<?=esc($data['siteName'])?>"><meta name="theme-color" content="#1a1a1a"><link rel="icon" href="https://www.5iehome.cc/favicon.ico"><link rel="dns-prefetch" href="https://t0.gstatic.cn"><link href="https://fonts.googleapis.com/css2?family=LXGW+WenKai:wght@400;700&display=swap" rel="stylesheet" media="print" onload="this.media='all'"><style>
*,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
html{scroll-behavior:smooth}
:root{--font-cn:'LXGW WenKai','PingFang SC','Microsoft YaHei',sans-serif;--sidebar-w:220px;--transition:0.2s ease;--danger:#e16162;--success:#00b894;--accent:#888}
...
body.admin-mode .nav-item{cursor:grab}
...
.bookmark-card{display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:6px;cursor:pointer;transition:background .15s ease,opacity .2s ease;text-decoration:none;position:relative;user-select:none;overflow:hidden}
.bookmark-card:hover{background:var(--card-hover)}
.bookmark-card:active{transition-duration:.08s}
body.admin-mode .bookmark-card{cursor:grab}
...
let fT=null;
function onFI(){const v=document.getElementById('bmF').value.trim();if(v){if(/\.(png|ico|svg|jpg|jpeg|webp|gif)$/i.test(v)){uFP(v)}else if(/^https?:\/\//i.test(v)){uFP(gFU(v))}else{uFP(v)}}else{const u=document.getElementById('bmU').value.trim();if(u&&/^https?:\/\//i.test(u))uFP(gFU(u))}}
...
function oBM(gid,bid){eB={groupId:gid,bookmarkId:bid||null};if(bid){fetch(`nav-api.php?action=getBookmark&groupId=${encodeURIComponent(gid)}&bookmarkId=${encodeURIComponent(bid)}`).then(r=>r.json()).then(bm=>{document.getElementById('bmMT').textContent='编辑书签';document.getElementById('bmU').value=bm.url;document.getElementById('bmN').value=bm.name;let fv=bm.favicon||'';if(fv&&/^https?:\/\//i.test(fv)&&!/\.(png|ico|svg|jpg|jpeg|webp|gif)$/i.test(fv))fv=gFU(fv);document.getElementById('bmF').value=fv;uFP(fv||gFU(bm.url));document.getElementById('bmM').classList.add('show');setTimeout(()=>document.getElementById('bmU').focus(),100)})}else{document.getElementById('bmMT').textContent='添加书签';document.getElementById('bmU').value='';document.getElementById('bmN').value='';document.getElementById('bmF').value='';document.getElementById('fP').innerHTML='<span style="color:var(--text3)">输入网址后自动获取</span>';document.getElementById('bmM').classList.add('show');setTimeout(()=>document.getElementById('bmU').focus(),100)}}
async function sBm(){const u=document.getElementById('bmU').value.trim(),n=document.getElementById('bmN').value.trim(),cf=document.getElementById('bmF').value.trim();if(!u||!n){sT('请输入网址和名称','error');return}const fu=/^https?:\/\//i.test(u)?u:'https://'+u;let fv=cf;if(fv&&/^https?:\/\//i.test(fv)&&!/\.(png|ico|svg|jpg|jpeg|webp|gif)$/i.test(fv))fv=gFU(fv);if(!fv)fv=gFU(fu);const f=new FormData();f.append('action','saveBookmark');f.append('groupId',eB.groupId);f.append('bookmarkId',eB.bookmarkId||'');f.append('url',fu);f.append('name',n);f.append('desc','');f.append('favicon',fv);try{const r=await fetch('nav-api.php',{method:'POST',body:f}),j=await r.json();if(j.success){cM('bmM');sT(eB.bookmarkId?'书签已更新':'书签已添加','success');setTimeout(()=>location.reload(),500)}else sT('保存失败','error')}catch(e){sT('保存失败','error')}}
function buildIconPicker(){const c=document.getElementById('iconPicker');c.innerHTML=ICON_KEYS.map((k,i)=>`<button class="icon-option${k===sIcon?' active':''}" data-icon="${k}" onclick="sIcon='${k}';buildIconPicker()">${ICON_SVGS[i]}</button>`).join('')}
function oGM(gid){eG=gid||null;buildIconPicker();if(gid){fetch(`nav-api.php?action=getGroup&groupId=${encodeURIComponent(gid)}`).then(r=>r.json()).then(g=>{document.getElementById('gMT').textContent='编辑分组';document.getElementById('gN').value=g.name;sIcon=g.icon||'home';buildIconPicker();document.getElementById('gM').classList.add('show');setTimeout(()=>document.getElementById('gN').focus(),100)})}else{document.getElementById('gMT').textContent='添加分组';document.getElementById('gN').value='';sIcon='home';buildIconPicker();document.getElementById('gM').classList.add('show');setTimeout(()=>document.getElementById('gN').focus(),100)}}
async function sGp(){const n=document.getElementById('gN').value.trim();if(!n){sT('请输入分组名称','error');return}const f=new FormData();f.append('action','saveGroup');f.append('groupId',eG||'');f.append('name',n);f.append('icon',sIcon);try{const r=await fetch('nav-api.php',{method:'POST',body:f}),j=await r.json();if(j.success){cM('gM');sT(eG?'分组已更新':'分组已添加','success');setTimeout(()=>location.reload(),500)}else sT('保存失败','error')}catch(e){sT('保存失败','error')}}
...
function onGroupDragOver(e){if(!dragGroup)return;e.preventDefault();e.dataTransfer.dropEffect='move'}
function onGroupDragEnter(e){if(!dragGroup)return;e.preventDefault();const t=e.currentTarget;if(t!==dragGroup)t.classList.add('drag-over')}
function onGroupDragLeave(e){e.currentTarget.classList.remove('drag-over')}
function onGroupDrop(e){e.preventDefault();const t=e.currentTarget;t.classList.remove('drag-over');if(!dragGroup||t===dragGroup)return;const dt=e.dataTransfer.getData('drag-type');if(dt==='group'){const c=document.getElementById('gC'),all=[...c.querySelectorAll('.group')],fi=all.indexOf(dragGroup),ti=all.indexOf(t);if(fi<ti)c.insertBefore(dragGroup,t.nextSibling);else c.insertBefore(dragGroup,t);sGOrder()}}
...
function sGOrder(){const order=[...document.querySelectorAll('.group')].map(g=>g.id.replace('g-',''));const f=new FormData();f.append('action','reorderGroups');f.append('order',JSON.stringify(order));fetch('nav-api.php',{method:'POST',body:f}).then(r=>r.json()).then(j=>{if(j.success)sT('分组顺序已保存','success');else sT('保存顺序失败','error')}).catch(()=>sT('保存顺序失败','error'))}
// ===== Sidebar nav drag =====
let dragNav=null;
function onNavDragStart(e){if(!document.body.classList.contains('admin-mode'))return;const btn=e.currentTarget.closest('.nav-item');if(!btn)return;dragNav=btn;btn.classList.add('dragging');e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain','');e.dataTransfer.setData('drag-type','nav')}
function onNavDragOver(e){if(!dragNav)return;e.preventDefault();e.dataTransfer.dropEffect='move'}
function onNavDragEnter(e){if(!dragNav)return;e.preventDefault();const t=e.currentTarget.closest('.nav-item');if(t&&t!==dragNav)t.classList.add('drag-over')}
function onNavDragLeave(e){const t=e.currentTarget.closest('.nav-item');if(t)t.classList.remove('drag-over')}
function onNavDrop(e){e.preventDefault();const t=e.currentTarget.closest('.nav-item');if(t)t.classList.remove('drag-over');if(!dragNav||!t||t===dragNav)return;const dt=e.dataTransfer.getData('drag-type');if(dt==='nav'){const c=document.getElementById('sNav'),all=[...c.querySelectorAll('.nav-item')],fi=all.indexOf(dragNav),ti=all.indexOf(t);if(fi<ti)c.insertBefore(dragNav,t.nextSibling);else c.insertBefore(dragNav,t);sNavOrder()}}
function onNavDragEnd(e){const t=e.currentTarget.closest('.nav-item');if(t)t.classList.remove('dragging');dragNav=null;document.querySelectorAll('.nav-item.drag-over').forEach(x=>x.classList.remove('drag-over'))}
...
let dragBm=null;
function onBmDragStart(e){if(!document.body.classList.contains('admin-mode'))return;const card=e.currentTarget.closest('.bookmark-card');if(!card)return;e.preventDefault();dragBm=card;card.classList.add('dragging-bm');e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain','');e.dataTransfer.setData('drag-type','bookmark');e.dataTransfer.setData('bm-gid',card.dataset.gid);e.dataTransfer.setData('bm-bid',card.dataset.bid)}
function onBmDragOver(e){if(!dragBm)return;e.preventDefault();e.dataTransfer.dropEffect='move';const card=e.currentTarget.closest('.bookmark-card');if(!card||card===dragBm)return;card.classList.remove('drop-before','drop-after');const rect=card.getBoundingClientRect();const mid=rect.top+rect.height/2;if(e.clientY<mid)card.classList.add('drop-before');else card.classList.add('drop-after')}
function onBmDragEnter(e){e.preventDefault()}
...
// No background-click-to-close modals - only button clicks
iT();
