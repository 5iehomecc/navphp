...
<?php
function renderSvgIcon($id){global $ICONS;$paths=$ICONS[$id]??$ICONS['home'];$out='';foreach($paths as $p){$out.='<'.$p.'>';}return '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">'.$out.'</svg>';}
$iconKeys=json_encode(array_keys($ICONS));
$iconSvgs=[];foreach(array_keys($ICONS) as $id){$p='';foreach($ICONS[$id] as $el){$p.='<'.$el.'>';}$iconSvgs[]='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">'.$p.'</svg>';}
$iconSvgMap=json_encode($iconSvgs);
?>
